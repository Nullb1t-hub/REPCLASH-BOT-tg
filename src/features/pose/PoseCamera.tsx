...
export function PoseCamera({enabled,exercise,onRep,onToggle,sessionReps}:Props){
 ...
 return <div className={"camera-card "+(enabled?"is-live":"")}><div className="camera-head"><div><span className="eyebrow">REPCLASH AI</span><b>{exercise==="pushup"?"ОТЖИМАНИЯ":"ПРИСЕДАНИЯ"}</b></div><div className="camera-balance"><small>SESSION</small><strong>{sessionReps}</strong></div></div><div className="camera-wrap"><video ref={videoRef} autoPlay muted playsInline/><div className="camera-overlay"/><div className="camera-scan"/>{!enabled&&<div className="camera-idle"><span>◉</span><b>AI READY</b><small>Техника → REP → баланс</small></div>} {error&&<div className="camera-error">{error}</div>}</div><div className="camera-status"><span className={"ai-dot "+(error?"bad":"")}>● {status}</span><span>{enabled?"КАМЕРА ВКЛЮЧЕНА":"КАМЕРА ВЫКЛЮЧЕНА"}</span></div><div className="camera-hint">{hint}</div><button className={"camera-start "+(enabled?"stop":"")} onClick={onToggle}>{enabled?<><span>■</span> ЗАКОНЧИТЬ СЕССИЮ · +{sessionReps} REP</>:<><span>◉</span> НАЧАТЬ ЗАРАБАТЫВАТЬ REP</>}</button><div className="camera-foot">Баланс пополнится только после завершения сессии. Один REP = один ход.</div></div>
}
